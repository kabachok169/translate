...
```
Есть и другие случаи, при которых `construct()` не сработает. [Пример 33.8]() пытыется создать массив типа `int` с 4096-ю элементами. Управляемая совместная память при этом вмещает только 1024 байта. Это вызывает исключение типа ` boost::interprocess::bad_alloc`.

После того, как объекты были созданы в управляемой общей памяти, они могут быть удалены с помощью метода `destroy()`.
